...
export default function AuthMenu() {
    ...
    return (
        <div className="w-full h-full flex flex-col items-center gap-4 bg-black text-white">
            <div className="flex">
                {mode == 0 && <span className="text-xl">Login</span>}
                {mode == 1 && <span className="text-xl">Register</span>}
            </div>
            <form className="w-3/4 flex flex-col gap-4 bg-gray-800 px-6 py-6 rounded-md" onSubmit={(...args) => {
                if (mode == 0)
                    loginUser(...args);
                ...
                <div className="h-20 flex flex-col gap-2">
                    ...
                    <input id="email" name="email" type="email" className="px-2 h-12 bg-gray-900 border border-white rounded-sm" />
                </div>
                <div className="h-20 flex flex-col gap-2">
                    <label className="text-lg" htmlFor="password">Password</label>
                    <input id="password" name="password" type="password" className="px-2 h-12 bg-gray-900 border border-white rounded-sm" />
                </div>
                {mode == 1 && (
                    <div className="h-20 flex flex-col gap-2">
                        <label className="text-lg" htmlFor="confirm_password">Confirm Password</label>
                        <input id="confirm_password" name="confirm_password" type="password" className="px-2 h-12 bg-gray-900 border border-white rounded-sm" />
                    </div>
                )}
                <div className="">
